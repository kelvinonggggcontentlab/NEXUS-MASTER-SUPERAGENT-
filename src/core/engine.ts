...
export class NexusCore {
  constructor(private readonly provider: NexusAIProvider, private readonly registry: NexusToolRegistry, private readonly permissions = new NexusPermissionEngine(), private readonly responses = new NexusResponseEngine(), private readonly maxIterations = 20) {}
  async run(request: string, mode: Mode, onEvent: (event: AgentEvent) => void, confirmed = true): Promise<{ execution: AgentExecution; response: string }> {
    ...
    for (let iteration = 0; iteration < this.maxIterations; iteration++) {
      ...
      await Promise.all(ready.map((item) => this.executeTask(item, execution, request, confirmed, emit)));
    }
  ...
  private async executeTask(task: AgentTask, execution: AgentExecution, request: string, confirmed: boolean, emit: (type: AgentEvent["type"], message: string) => void): Promise<void> {
    ...
    task.status = "running"; emit("status", `${task.label}…`); let output: ToolResult | undefined;
    while (task.attempts < MAX_RETRIES) { task.attempts++; output = await tool.execute(task.input, { executionId: execution.executionId, idempotencyKey: `${execution.executionId}:${task.id}`, mode: execution.mode }); if (output.success || !output.error?.retryable) break; await new Promise((resolve) => setTimeout(resolve, 20 * 2 ** task.attempts)); }
