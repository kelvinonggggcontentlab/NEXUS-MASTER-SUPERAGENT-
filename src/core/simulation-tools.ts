...
const tool = (id: string, name: string, permission: "read" | "write", execute: (input: Record<string, unknown>, context: ToolContext) => unknown): NexusTool => ({ id, name, description: `Simulated ${name} adapter`, category: "Google", capability: id, authentication: "user_oauth", permission, riskLevel: permission === "write" ? "medium" : "low", enabled: true, requiresConfirmation: false, inputSchema: objectInput, execute: async (input, context) => result(name, context, execute(input, context)), verify: permission === "write" ? verified : undefined });
export const simulationTools: NexusTool[] = [
  ...
  tool("document.summarize", "Document summary", "read", (input) => ({ summary: `Key points extracted from ${String(input.fileId ?? "the document")}: invoice details, payment amount, and due date.` })),
  tool("gmail.send", "Gmail send", "write", (input) => ({ messageId: "sent-invoice-sept", recipient: String(input.to), subject: String(input.subject) })),
  tool("calendar.availability", "Google Calendar availability", "read", () => ({ slots: ["Tomorrow, 10:00–11:00", "Tomorrow, 14:00–15:00"] })),
